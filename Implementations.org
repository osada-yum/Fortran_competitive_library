...
#+begin_src fortran :exports code
  ...
  pure integer(int32) function lower_bound_${suffix}(arr, val)
    ${type}, intent(in) :: arr(:)
    ${type}, intent(in) :: val
    integer(int32) :: p, q, r
    p = 1
...
#+begin_src fortran :exports code :tangle "test/test_lower_bound.F90" :noweb no-export
  program test_lower_bound
    ...
    ! arr2
    arr2(1) = 7
    <<assert(cond="lower_bound(arr2, 6) == 1",code=12,message="`lower_bound` does not work well for one element array...")>>
    <<assert(cond="lower_bound(arr2, 7) == 1",code=12,message="`lower_bound` does not work well for one element array...")>>
    <<assert(cond="lower_bound(arr2, 8) == 2",code=12,message="`lower_bound` does not work well for one element array...")>>
    ! dup_arr
    dup_arr = [1, 1, 2, 3, 3, 3, 3, 5, 5, 5]
    <<assert(cond="lower_bound(dup_arr, 0) == 1",code=13,message="`lower_bound` does not work well for the array that has same values...")>>
    <<assert(cond="lower_bound(dup_arr, 2) == 3",code=13,message="`lower_bound` does not work well for the array that has same values...")>>
    <<assert(cond="lower_bound(dup_arr, 3) == 4",code=13,message="`lower_bound` does not work well for the array that has same values...")>>
    <<assert(cond="lower_bound(dup_arr, 5) == 8",code=13,message="`lower_bound` does not work well for the array that has same values...")>>
    <<assert(cond="lower_bound(dup_arr, 7) == 11",code=13,message="`lower_bound` does not work well for the array that has same values...")>>
    ! allsame_arr
    allsame_arr = [(1, i = 1, n)]
    <<assert(cond="lower_bound(allsame_arr, 0) == 1",code=13,message="`lower_bound` does not work well for the array that has all same values...")>>
    <<assert(cond="lower_bound(allsame_arr, 1) == 1",code=13,message="`lower_bound` does not work well for the array that has all same values...")>>
    <<assert(cond="lower_bound(allsame_arr, 2) == 11",code=13,message="`lower_bound` does not work well for the array that has all same values...")>>
  end program test_lower_bound
#+end_src
#+name: test-vector
#+begin_src fortran :flags "-cpp -g -fbacktrace" :exports both :results output :noweb no-export :cache yes
<<lower_bound-module>>
<<lower_bound-test>>
#+end_src

#+RESULTS[0565c652816f22840ea4e3e4271b629849a852f1]: test-vector
...
#+end_src
#+name: test-tuple2
#+begin_src fortran :flags "-cpp -g -fbacktrace -fcheck=bounds" :exports both :results output :noweb no-export :cache yes
<<tuple2-module>>
<<tuple2-test>>
#+end_src
...
#+begin_src fortran :exports code :tangle "test/test_vector.F90" :noweb no-export
  program test_vector
    ...
    do i = 0, v%size()+1
       j = v%lower_bound(i)
       <<assert-false(cond="j == max(1, i)", code=11, message="Return value of `lower_bound` is illegal in loop.")>>
    end do
    do i = 1, n
       <<assert-false(cond="v%at(i) == i" ,code=12)>>
       call v%replace(i, -i)
       <<assert-false(cond="v%at(i) == -i" ,code=13)>>
    end do
    j = v%at(n+1, ierr)
    <<assert-true(cond="ierr == 0", code=14, message="Return value of `at` is illegal.")>>
    do i = 1, n
       j = v%pop_back()
    end do
    j = v%pop_back(ierr)
    <<assert-true(cond="ierr == 0", code=15, message="Return value of `pop_back` is illegal.")>>
    ...
    do i = low-1, high+1
       j = v2%lower_bound(i)
       <<assert-false(cond="j == max(low, i)", code=21, message="Return value of `lower_bound` is illegal in loop.")>>
    end do
    do i = low, high
       <<assert-false(cond="v2%at(i) == i" ,code=22)>>
       call v2%replace(i, -i)
       <<assert-false(cond="v2%at(i) == -i" ,code=23)>>
    end do
    j = v2%at(high+1, ierr)
    <<assert-true(cond="ierr == 0", code=24, message="Return value of `at` is illegal.")>>
    do i = 1, n
       j = v2%pop_back()
    end do
    j = v2%pop_back(ierr)
    <<assert-true(cond="ierr == 0", code=25, message="Return value of `pop_back` is illegal.")>>
...
#+begin_src fortran :flags -g -fbacktrace :exports both :results output :noweb no-export :cache yes
<<stream_out-module>>
<<stream_out-test>>
#+end_src
