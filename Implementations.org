...
#+begin_src fortran :exports code
  ...
  subroutine merge_with_key_${suffix}(key, indices, p, q, r)
    ...
    ${type} :: Left_key(1:q-p+1), Right_key(1:r-q)
    integer(int32) :: l_max, r_max
    l_max = q-p+1
    r_max = r-q
    block
...
#+RESULTS[90512ca95c869ead0fff5d7179475e6f0a8018e9]:
...
#+RESULTS[0fe41ea123767751694d6cd559bb76b8f9343a71]: test-upper_bound
...
#+begin_src fortran :exports code
  module string_m
    ...
    type :: string
     ...
     contains
       procedure, pass :: assign_string, assign_chars
       generic :: assignment(=) => assign_string
       final :: destroy_string
    end type string
    interface assignment(=)
       module procedure :: assign_string_to_chars
    end interface assignment(=)
    private equal_string, not_equal_string
    private less_string, less_equal_string, greater_string, greater_equal_string
    interface operator(==)
       module procedure :: equal_string
    end interface operator(==)
    interface operator(/=)
       module procedure :: not_equal_string
    end interface operator(/=)
    interface operator(<)
       module procedure :: less_string
    end interface operator(<)
    interface operator(<=)
       module procedure :: less_equal_string
    end interface operator(<=)
    interface operator(>)
       module procedure :: greater_string
    end interface operator(>)
    interface operator(>=)
       module procedure :: greater_equal_string
    end interface operator(>=)
  contains
    type(string) function to_string(chars) result(res)
      character(len=*), intent(in) :: chars
      type(string_row), target :: str_row
      res%size_ = len(chars)
      allocate(str_row%str_, source = chars)
      str_row%ref_cnt_ = 1_int32
    end function to_string
    subroutine destroy_string(str_row)
      class(string_row), intent(inout) :: str_row
    ...
    subroutine assign_chars(this, chars)
      class(string), intent(inout) :: this
      character(len=*), intent(in) :: chars
      this%size_ = len(chars)
      this%str_ = chars
    end subroutine assign_chars
    subroutine assign_string_to_chars(chars, str)
      character(len=*), intent(inout) :: chars
      type(string), intent(in) :: str
      if (len(chars) < str%size_) then
         chars(1:len(chars)) = str%str_(1:len(chars))
      else
         chars(1:str%size_) = str%str_
      end if
    end subroutine assign_string_to_chars
    pure logical function equal_string(lhs, rhs) result(res)
      type(string), intent(in) :: lhs, rhs
      res = lhs%size_ == rhs%size_ .and. lhs%str_ == rhs%str_
    end function equal_string
    pure logical function not_equal_string(lhs, rhs) result(res)
      type(string), intent(in) :: lhs, rhs
      res = .not. (lhs == rhs)
    end function not_equal_string
    pure logical function less_string(lhs, rhs) result(res)
      type(string), intent(in) :: lhs, rhs
      res = lhs%size_ < rhs%size_ .or. (lhs%size_ == rhs%size_ .and. lhs%str_ < rhs%str_)
    end function less_string
    pure logical function less_equal_string(lhs, rhs) result(res)
      type(string), intent(in) :: lhs, rhs
      res = lhs < rhs .or. lhs == rhs
    end function less_equal_string
    pure logical function greater_string(lhs, rhs) result(res)
      type(string), intent(in) :: lhs, rhs
      res = .not. (lhs <= rhs)
    end function greater_string
    pure logical function greater_equal_string(lhs, rhs) result(res)
      type(string), intent(in) :: lhs, rhs
      res = .not. (lhs < rhs)
    end function greater_equal_string
  end module string_m
#+end_src
** Tuple
*** Tuple2
...
#+begin_src fortran :exports code
  ...
  type :: ${tuple2}
     ...
     ${type2} :: snd_
   contains
     procedure, pass :: fst  => fst_${tuple2}
     procedure, pass :: snd  => snd_${tuple2}
  end type ${tuple2}
...
#+end_src
***** fst
=function fst= は Tuple2 の一番目の要素を返す.
#+name: fst-tuple2
#+begin_src fortran :exports code
  !> fst_${tuple2}: Return the first element of ${tuple2}.
  ${type1} function fst_${tuple2}(this) result(res)
    class(${tuple2}), intent(in) :: this
    res = this%fst_
  end function fst_${tuple2}
#+end_src
***** snd
=function snd= は Tuple2 の二番目の要素を返す.
#+name: snd-tuple2
#+begin_src fortran :exports code
  !> snd_${tuple2}: Return the second element of ${tuple2}.
  ${type1} function snd_${tuple2}(this) result(res)
    class(${tuple2}), intent(in) :: this
    res = this%snd_
  end function snd_${tuple2}
#+end_src
***** compare_operator
比較演算子たち.
#+name: compare-tuple2
#+begin_src fortran :exports code :noweb no-export
  ...
  logical function less_${tuple2}(lhs, rhs) result(res)
  ...
  logical function less_equal_${tuple2}(lhs, rhs) result(res)
    type(${tuple2}), intent(in) :: lhs, rhs
    res = lhs%fst_ < rhs%fst_
    if (lhs%fst_ == rhs%fst_) then
       res = lhs%snd_ <= rhs%snd_
  ...
  logical function greater_${tuple2}(lhs, rhs) result(res)
    type(${tuple2}), intent(in) :: lhs, rhs
    res = lhs%fst_ > rhs%fst_
    if (lhs%fst_ == rhs%fst_) then
       res = lhs%snd_ > rhs%snd_
  ...
  logical function greater_equal_${tuple2}(lhs, rhs) result(res)
    type(${tuple2}), intent(in) :: lhs, rhs
    res = lhs%fst_ > rhs%fst_
    if (lhs%fst_ == rhs%fst_) then
       res = lhs%snd_ >= rhs%snd_
  ...
  logical function equal_${tuple2}(lhs, rhs) result(res)
    type(${tuple2}), intent(in) :: lhs, rhs
    res = lhs%fst_ == rhs%fst_ .and. lhs%snd_ == rhs%snd_
  end function equal_${tuple2}
  logical function not_equal_${tuple2}(lhs, rhs) result(res)
    type(${tuple2}), intent(in) :: lhs, rhs
...
#+RESULTS[d6ff89889b87909e33444adf30a2eeb71bd211a9]: test-tuple2
...
#+begin_src fortran :exports code
  ...
  type :: ${tuple3}
     ...
     ${type3} :: thr_
   contains
     procedure, pass :: fst  => fst_${tuple3}
     procedure, pass :: snd  => snd_${tuple3}
     procedure, pass :: thr  => thr_${tuple3}
  end type ${tuple3}
...
#+end_src
***** fst
=function fst= は Tuple3 の一番目の要素を返す.
#+name: fst-tuple3
#+begin_src fortran :exports code
  !> fst_${tuple3}: Return the first element of ${tuple3}.
  ${type1} function fst_${tuple3}(this) result(res)
    class(${tuple3}), intent(in) :: this
    res = this%fst_
  end function fst_${tuple3}
#+end_src
***** snd
=function snd= は Tuple3 の二番目の要素を返す.
#+name: snd-tuple3
#+begin_src fortran :exports code
  !> snd_${tuple3}: Return the second element of ${tuple3}.
  ${type1} function snd_${tuple3}(this) result(res)
    class(${tuple3}), intent(in) :: this
    res = this%snd_
  end function snd_${tuple3}
#+end_src
***** thr
=function thr= は Tuple3 の三番目の要素を返す.
#+name: snd-tuple3
#+begin_src fortran :exports code
  !> thr_${tuple3}: Return the second element of ${tuple3}.
  ${type1} function thr_${tuple3}(this) result(res)
    class(${tuple3}), intent(in) :: this
    res = this%thr_
  end function thr_${tuple3}
#+end_src
***** compare_operator
比較演算子たち.
#+name: compare-tuple3
...
#+RESULTS[dd3d26909f4b23a4e97114ec4e76bc094ff5f3ea]: test-tuple3
...
#+begin_src fortran :exports code
  ...
  type :: ${uwvec}
   ...
   contains
     ...
     procedure, pass :: push_back_${uwvec}, push_back_array_${uwvec}
     generic         :: push_back => push_back_${uwvec}, push_back_array_${uwvec}
...
#+RESULTS[d0505353c7380d864b2e3f430aaea5ec571bf268]: test-priority_queue
...
module vec_deque_m
  ...
  type :: vec_dequeue
   ...
   contains
     procedure, pass :: init => init_vec_dequeue
     procedure, pass :: resize => resize_vec_dequeue
     procedure, pass :: push_front => push_front_vec_dequeue
     procedure, pass :: push_back  => push_back_vec_dequeue
     procedure, pass :: pop_front  => pop_front_vec_dequeue
     procedure, pass :: pop_back   => pop_back_vec_dequeue
     procedure, pass :: to_array => to_array_vec_dequeue
     procedure, pass :: debug_print => debug_print_vec_dequeue
  end type vec_dequeue
contains
  subroutine init_vec_dequeue(this)
    class(vec_dequeue), intent(inout) :: this
    ...
    this%tail_ = 1
  end subroutine init_vec_dequeue
  subroutine resize_vec_dequeue(this, capa)
    class(vec_dequeue), intent(inout) :: this
    ...
    this%capa_ = capa
  end subroutine resize_vec_dequeue
  subroutine push_front_vec_dequeue(this, v)
    class(vec_dequeue), intent(inout) :: this
    integer(int32), intent(in) :: v
    if (this%head_ == this%tail_) then
    ...
    this%size_ = this%size_ + 1
  end subroutine push_front_vec_dequeue
  subroutine push_back_vec_dequeue(this, v)
    class(vec_dequeue), intent(inout) :: this
    integer(int32), intent(in) :: v
    if (this%head_ == this%tail_) then
       call this%resize(this%capa_*2)
    ...
    this%size_ = this%size_ + 1
  end subroutine push_back_vec_dequeue
  integer(int32) function pop_front_vec_dequeue(this) result(res)
    class(vec_dequeue), intent(inout) :: this
    if (this%size_ == 0) then
       write(error_unit, '(a)') "Error in pop_front: vec_dequeue is empty."
       error stop 1
    end if
    this%size_ = this%size_ - 1
    this%head_ = this%head_ + 1
  ...
  end function pop_front_vec_dequeue
  integer(int32) function pop_back_vec_dequeue(this) result(res)
    class(vec_dequeue), intent(inout) :: this
    if (this%size_ == 0) then
       write(error_unit, '(a)') "Error in pop_back: vec_dequeue is empty."
       error stop 1
    end if
    this%size_ = this%size_ - 1
    this%tail_ = this%tail_ - 1
  ...
  end function pop_back_vec_dequeue
  pure function to_array_vec_dequeue(this) result(res)
    class(vec_dequeue), intent(in) :: this
  ...
  end function to_array_vec_dequeue
  subroutine debug_print_vec_dequeue(this, unit)
    class(vec_dequeue), intent(in) :: this
    integer(int32), intent(in) :: unit
    write(error_unit, '(2(i0, 1x), a, *(i0, 1x))') this%head_, this%tail_, ": ", this%arr_(:)
    write(unit, '(*(i0, 1x))') this%to_array()
  end subroutine debug_print_vec_dequeue
...
module btree_m
  ...
  type :: btree_node_ptr
   ...
   contains
     procedure, pass :: size => size_btree_node_ptr
     procedure, pass :: is_leaf => is_leaf_btree_node_ptr
     procedure, pass :: get => get_btree_node_ptr
     procedure, pass :: split_child => split_child_btree_node_ptr
     procedure, pass :: insert => insert_btree_node_ptr
     procedure, pass :: remove => remove_btree_node_ptr
     procedure, pass :: shrink_left  => shrink_left_btree_node_ptr
     procedure, pass :: expand_right => expand_right_btree_node_ptr
     procedure, pass :: print => print_btree_node_ptr
     procedure, pass :: check_invariant => check_invariant_btree_node_ptr
  ...
  type :: btree
   ...
   contains
     procedure, pass :: size   => size_btree
     procedure, pass :: height => height_btree
     procedure, pass :: init   => init_btree
     procedure, pass :: get    => get_btree
     procedure, pass :: get_iter => get_iter_btree
     procedure, pass :: contains => contains_btree
     procedure, pass :: insert => insert_btree
     procedure, pass :: remove => remove_btree
     procedure, pass :: minimum => minimum_btree
     procedure, pass :: maximum => maximum_btree
     ! procedure, pass :: lower_bound => lower_bound_btree
     ! procedure, pass :: upper_bound => upper_bound_btree
     procedure, pass :: print => print_btree
     procedure, pass :: check_invariant => check_invariant_btree
  end type btree
  ...
  type :: btree_node_iter
   ...
   contains
     procedure, pass :: key => key_btree_node_iter
     procedure, pass :: val => val_btree_node_iter
     procedure, pass :: mut => mut_btree_node_iter
     procedure, pass :: next => next_btree_node_iter
     procedure, pass :: prev => prev_btree_node_iter
     procedure, pass :: is_begin => is_begin_btree_node_iter
     procedure, pass :: is_end   => is_end_btree_node_iter
     procedure, pass :: exist => exist_btree_node_iter
  end type btree_node_iter
contains
  subroutine init_btree(this)
    ...
    this%height_ = 0
    this%root_%p_ => x
  end subroutine init_btree
  pure integer(int32) function size_btree(this) result(res)
    class(btree), intent(in) :: this
    res = this%size_
  end function size_btree
  pure integer(int32) function height_btree(this) result(res)
    class(btree), intent(in) :: this
    res = this%height_
  end function height_btree
  integer(int32) function get_btree(this, key) result(res)
    class(btree), intent(in) :: this
    integer(int32), intent(in) :: key
    type(btree_node_iter) :: iter
    iter = this%root_%get(key)
    if (iter%idx_ /= -1) then
       res = iter%nptr_%p_%val_(iter%idx_)
    else
       res = -1
    end if
  end function get_btree
  type(btree_node_iter) function get_iter_btree(this, key) result(res)
    class(btree), intent(in) :: this
    integer(int32), intent(in) :: key
    res = this%root_%get(key)
  end function get_iter_btree
  logical function contains_btree(this, key) result(res)
    class(btree), intent(in) :: this
    integer(int32), intent(in) :: key
    type(btree_node_iter) :: iter
    iter = this%root_%get(key)
    res = iter%idx_ /= -1
  end function contains_btree

  subroutine insert_btree(this, key, val)
    class(btree), intent(inout) :: this
    integer(int32), intent(in) :: key
    integer(int32), intent(in) :: val
  ...
  subroutine remove_btree(this, key)
    class(btree), intent(inout) :: this
    integer(int32), intent(in) :: key
    ...
    this%size_ = this%size_ - 1
  end subroutine remove_btree
  !> minimum_btree: Return the iterator to node that has minimum key.
  type(btree_node_iter) function minimum_btree(this) result(res)
    class(btree), intent(in) :: this
    res%nptr_%p_ => this%root_%p_
    res%depth_ = 1
    res%idx_ = 0
    call res%next()
  end function minimum_btree
  !> maximum_btree: Return the iterator to node that has maximum key.
  type(btree_node_iter) function maximum_btree(this) result(res)
    class(btree), intent(in) :: this
    res%nptr_%p_ => this%root_%p_
    res%depth_ = 1
    res%idx_ = res%nptr_%size() + 1
    call res%prev()
  end function maximum_btree

  !> print_btree: Print whole node in B-tree for debug.
  !> For debug.
  subroutine print_btree(this, unit)
    class(btree), intent(in) :: this
    integer(int32), intent(in) :: unit
    if (associated(this%root_%p_)) &
         call this%root_%print(unit, 0)
  end subroutine print_btree
  !> check_invariant_btree: Check invariant for debug.
  ...
  subroutine check_invariant_btree(this)
    class(btree), intent(in) :: this
    integer(int32) :: i
    ...
    bt_iter = this%minimum()
    k_bef = -huge(0_int32) - 1
  ...
  end subroutine check_invariant_btree

  pure integer(int32) function size_btree_node_ptr(this) result(res)
    class(btree_node_ptr), intent(in) :: this
    res = this%p_%nelem_
  end function size_btree_node_ptr
  pure logical function is_leaf_btree_node_ptr(this) result(res)
    class(btree_node_ptr), intent(in) :: this
    res = this%p_%is_leaf_
  end function is_leaf_btree_node_ptr
  type(btree_node_iter) function get_btree_node_ptr(this, key) result(res)
    class(btree_node_ptr), intent(in) :: this
    integer(int32), intent(in) :: key
    integer(int32) :: pos
    ...
    res%idx_ = -1
    return
  end function get_btree_node_ptr
  subroutine split_child_btree_node_ptr(this, idx)
    class(btree_node_ptr), intent(in) :: this
  ...
  end subroutine split_child_btree_node_ptr
  !> insert_btree_node_ptr: Insert (`key`, `val`) into B-tree.
  type(btree_node_iter) function insert_btree_node_ptr(this, key, val) result(res)
    class(btree_node_ptr), intent(in) :: this
    integer(int32), intent(in) :: key
    integer(int32), intent(in) :: val
    ...
    res%nptr_%p_ => x%p_
    res%idx_ = pos
  end function insert_btree_node_ptr
  !> remove_btree_node_ptr: Remove `key` from B-tree.
  !> invariant condition: the node `this%p_` has at least `t` keys.
  recursive subroutine remove_btree_node_ptr(this, key)
    class(btree_node_ptr), intent(in) :: this
    integer(int32), intent(in) :: key
    ...
    if (x%p_%key_(min(pos, x%size())) == key) then !> `key` exists in current internal node.
       call remove_key_btree_node_ptr(x, key, pos)
       return
    end if
    ...
    if (pos == s + 1) then
       if (x%p_%children_(s)%size() == t - 1) then
          ...
          !>       \
          !>         (c_to//key//c_from)
          call merge_children_btree_node_ptr(x, s)
       ...
       if (x%p_%children_(pos+1)%size() == t - 1) then
          ...
          !>                   /   \
          !> c(c_to//x//c_from)      c3
          call merge_children_btree_node_ptr(x, pos)
    ...
    if (pos == s + 1) then
       ...
       !>          /    \
       !>   c_from        (x:c_to)
       call rotate_right_btree_note_ptr(x, s)
       call x%p_%children_(x%size() + 1)%remove(key)
    else !> pos: [1, s]
       ...
       !>          v1
       !> (c_to:x)    c_from
       call rotate_left_btree_note_ptr(x, pos)
       call x%p_%children_(pos)%remove(key)
    end if
    ! write(error_unit, '(a, i0)') "pattern 3a end: ", key
    return
  end subroutine remove_btree_node_ptr
  !> remove_key_btree_node_ptr: If some of current nodes have `key`, call this.
  recursive subroutine remove_key_btree_node_ptr(x, key, pos)
    type(btree_node_ptr), intent(in) :: x
    integer(int32), intent(in) :: key
    integer(int32), intent(in) :: pos
    !> pos: [1, x%size()].
    if (x%is_leaf()) then
       ! write(error_unit, '(a, i0)') "pattern 1: ", key
       call x%shrink_left(pos, -1)
       return
    end if
    !> x is not leaf.
  ...
  end subroutine remove_key_btree_node_ptr

  !> shrink_left_btree_node_ptr: Remove the `key` and `val` from the `pos` of `this%p_` and shrink it.
  !> before: key(1), key(2), ... key(pos-1), key(pos), key(pos+1), ..., key(s)
  !> after : key(1), key(2), ... key(pos-1), key(pos+1), ..., key(s)
  !> unlink left pointer if `left_or_right` == 0
  !> unlink right pointer if `left_or_right` == 1
  !> `this%p_` is leaf if `left_or_right` == -1.
  subroutine shrink_left_btree_node_ptr(this, pos, left_or_right)
    class(btree_node_ptr), intent(in) :: this
    integer(int32), intent(in) :: pos
    integer(int32), intent(in) :: left_or_right
    type(btree_node_ptr) :: x
    integer(int32) :: i, s
    x%p_ => this%p_
    s = x%size()
    ...
    if (left_or_right /= -1) then
       do i = pos + left_or_right, s
          x%p_%children_(i)%p_ => x%p_%children_(i+1)%p_
       end do
    end if
    x%p_%nelem_ = s - 1
  end subroutine shrink_left_btree_node_ptr
  subroutine expand_right_btree_node_ptr(this, pos)
    class(btree_node_ptr), intent(in) :: this
    integer(int32), intent(in) :: pos
    type(btree_node_ptr) :: x
    integer(int32) :: i, s
    ...
    x%p_%nelem_ = s + 1
  end subroutine expand_right_btree_node_ptr
  !> merge_btree_node_ptr: Merge left child, middle key and right child.
  ...
  !>                  _y
  !> (left//x//right)    _c
  subroutine merge_children_btree_node_ptr(x, pos)
    type(btree_node_ptr), intent(in) :: x
    integer(int32), intent(in) :: pos
    ...
    deallocate(right%p_)
    nullify(right%p_)
    call x%shrink_left(pos, 1) ! unlink right child.
  end subroutine merge_children_btree_node_ptr
  !> rotate_right_btree_note_ptr: Rotate keys.
  ...
  !>        /    \
  !>   init        (key:right)
  subroutine rotate_right_btree_note_ptr(x, pos)
    type(btree_node_ptr), intent(in) :: x
    integer(int32), intent(in) :: pos
    type(btree_node_ptr) :: left, right
    left%p_  => x%p_%children_(pos)%p_
    ...
    call left%shrink_left(left%size(), 1)
  end subroutine rotate_right_btree_note_ptr
  !> rotate_left_btree_note_ptr: Rotate keys.
  ...
  !>            /    \
  !> (left:key)        rest
  subroutine rotate_left_btree_note_ptr(x, pos)
    type(btree_node_ptr), intent(in) :: x
    integer(int32), intent(in) :: pos
    type(btree_node_ptr) :: left, right
    integer(int32) :: ls
    ...
    call right%shrink_left(1, 0) !> right%size() -= 1
  end subroutine rotate_left_btree_note_ptr

  recursive subroutine print_btree_node_ptr(this, unit, depth)
    class(btree_node_ptr), intent(in) :: this
    integer(int32), intent(in) :: unit, depth
    type(btree_node_ptr) :: x
    integer(int32) :: i
    x%p_ => this%p_
    ! write(unit, '(L, 1x, i0, a, *(i0, 1x))') associated(x%p_), x%size(), ": ", x%p_%key_(:)
    write(unit, '(a, i0, a, *(i0, 1x))') repeat("|", min(1, depth))//repeat("-", depth), depth, ": ", x%p_%key_(1:x%size())
    if (x%is_leaf()) return
    do i = 1, x%p_%nelem_ + 1
       call x%p_%children_(i)%print(unit, depth + 1)
    end do
  end subroutine print_btree_node_ptr
  recursive subroutine check_invariant_btree_node_ptr(this)
    class(btree_node_ptr), intent(in) :: this
    integer(int32) :: i
  ...
  end subroutine check_invariant_btree_node_ptr

  pure integer(int32) function key_btree_node_iter(this) result(res)
    class(btree_node_iter), intent(in) :: this
    res = this%nptr_%p_%key_(this%idx_)
  end function key_btree_node_iter
  pure integer(int32) function val_btree_node_iter(this) result(res)
    class(btree_node_iter), intent(in) :: this
    res = this%nptr_%p_%val_(this%idx_)
  end function val_btree_node_iter
  subroutine mut_btree_node_iter(this, val)
    class(btree_node_iter), intent(in) :: this
    integer(int32), intent(in) :: val
    type(btree_node_ptr) :: x
    x%p_ => this%nptr_%p_
    x%p_%val_(this%idx_) = val
  end subroutine mut_btree_node_iter
  subroutine prev_btree_node_iter(this)
  ...
  end subroutine prev_btree_node_iter
  subroutine next_btree_node_iter(this)
    class(btree_node_iter), intent(inout) :: this
    if (this%is_end()) then
    ...
    if (this%nptr_%is_leaf()) then
       ...
       do !> visit parent of current node if `this%idx_` exceeds the range of `this%nptr_%p_%key_(1:s)`, where s == this%nptr_%size().
          if (this%depth_ == 1) return !> end of iterator if `this%idx_ == this%nptr_%size() + 1` and `this%p_` is root of B-tree.
          this%depth_ = this%depth_ - 1
          this%nptr_%p_ => this%parents_(this%depth_)%p_
          nullify(this%parents_(this%depth_)%p_)
          this%idx_ = this%indices_(this%depth_) + 1
  ...
  end subroutine next_btree_node_iter
  !> is_begin_btree_node_iter: return iter is begining of B-tree.
  logical function is_begin_btree_node_iter(this) result(res)
    class(btree_node_iter), intent(in) :: this
    res = this%depth_ == 1 .and. this%idx_ == 0
  end function is_begin_btree_node_iter
  !> is_end_btree_node_iter: return iter is end of B-tree.
  logical function is_end_btree_node_iter(this) result(res)
    class(btree_node_iter), intent(in) :: this
    res = this%depth_ == 1 .and. this%idx_ == this%nptr_%size() + 1
  end function is_end_btree_node_iter
  logical function exist_btree_node_iter(this) result(res)
    class(btree_node_iter), intent(in) :: this
    res = this%idx_ /= -1
  end function exist_btree_node_iter
...
program test_btree_m
  ...
  implicit none
  type(btree) :: m
  integer(int32), parameter :: n = 3*10**7, print_period = n/10
  call m%init()
  write(output_unit, '(a, *(i0, 1x))') "(random insertion): s, h: ", m%size(), m%height()
  call check_insertion_random(n)
  write(output_unit, '(a, *(i0, 1x))') "(ascending insertion): s, h: ", m%size(), m%height()
  call check_insertion_ascending(n)
  write(output_unit, '(a, *(i0, 1x))') "(descending insertion): s, h: ", m%size(), m%height()
  call check_insertion_descending(n)
  write(output_unit, '(a, *(i0, 1x))') "(all done): s, h: ", m%size(), m%height()
...
contains
  subroutine check_insertion_random(n)
    ...
    do i = 1, n
       ...
       if (mod(i, print_period) == 0) then
          write(output_unit, '(a, i0, a)') "insert: ", i
    ...
    !   bt_iter = m%minimum()
    !   do while (.not. bt_iter%is_end())
    !      write(error_unit, '(i0, 1x)', advance = "no") bt_iter%key()
    !      call bt_iter%next()
    !   end do
    !   write(error_unit, *)
    !   call m%print(error_unit)
    ...
    do i = 1, n
       if (mod(i, print_period) == 0) then
          write(output_unit, '(a, i0)') "get: ", i
          call m%check_invariant()
       end if
       if (.not. m%get(arr(i)) == i) then
          write(error_unit, '(a)') "insertion is error"
          error stop 4
       end if
    end do
    do i = 1, n
       if (mod(i, print_period) == 0) then
          write(output_unit, '(i0, a, a, 2(i0, 1x), a, i0)') i, ": ", "s, h: ", m%size(), m%height(), " remove: ", arr(i)
          call m%check_invariant()
       end if
       ! v = m%get(arr(i))
       ! write(error_unit, '(a, i0)') "exists: ", v
       call m%remove(arr(i))
       ! if (mod(i, print_period) == 0) then
       ! call m%print(error_unit)
       ! end if
  ...
  subroutine check_insertion_ascending(n)
    ...
    do i = 1, n
       if (mod(i, print_period) == 0) then
          write(output_unit, '(a, i0)') "insert: ", i
          call m%check_invariant()
       end if
       call m%insert(i, i)
    end do
    write(output_unit, '(a, *(i0, 1x))') "s, h: ", m%size(), m%height()
    do i = 1, n
       if (mod(i, print_period) == 0) then
          write(output_unit, '(a, i0)') "remove: ", i
          call m%check_invariant()
       end if
       call m%remove(i)
    end do
...
#+RESULTS[30f6c1d3311cee3d73d32a8d360d0cba76f9f03f]: test-binary_indexed_tree
...
#+begin_src fortran :exports code :noweb no-export
  module modint_m
    ...
    implicit none
    private
    integer(int64) :: modulo
    public :: modint
    type :: modint
       integer(int64) :: val_
     contains
       procedure, pass :: assign_m_from_i32, assign_m_from_i64, assign_m_from_m
       generic :: assignment(=) => assign_m_from_i32, assign_m_from_i64, assign_m_from_m
    end type modint
    interface operator(+)
    ...
    interface inv
       module procedure :: inv_modint, inv_int32, inv_int64
    end interface inv

    public :: modint_set_modulo
    interface modint_set_modulo
       module procedure :: modint_set_modulo_int32, modint_set_modulo_int64
    end interface modint_set_modulo
  contains
    subroutine modint_set_modulo_int32(m)
      integer(int32), intent(in) :: m
      modulo = m
    end subroutine modint_set_modulo_int32
    subroutine modint_set_modulo_int64(m)
      integer(int64), intent(in) :: m
      modulo = m
    end subroutine modint_set_modulo_int64
  end module modint_m
#+end_src
*** procedures
