...
#+begin_src fortran :exports code :tangle "test/test_lower_bound.F90" :noweb no-export
  program test_lower_bound
    ...
    <<assert(cond="j > size(arr)",code=12,message="`lower_bound` does not work well...")>>
    ! arr2
    arr2(1) = 7
    ...
    <<assert(cond="lower_bound(arr2, 8) > size(arr2)",code=15,message="`lower_bound` does not work well for one element array...")>>
    ! dup_arr
...
#+RESULTS[1c7fcd6441afbdf53e6b07a00585dbbe409d0bee]: test-lower_bound
