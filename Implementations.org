...
#+begin_src fortran :exports code :tangle "test/test_lower_bound.F90" :noweb no-export
  program test_lower_bound
    ...
    do i = 0, n+1
       j = lower_bound(arr, i)
       <<assert-false(cond="j == max(1, i)",code=11,message="`lower_bound` does not work well...")>>
    end do
    ! arr2
    arr2(1) = 7
    <<assert-false(cond="lower_bound(arr2, 6) == 1",code=12,message="`lower_bound` does not work well for one element array...")>>
    <<assert-false(cond="lower_bound(arr2, 7) == 1",code=12,message="`lower_bound` does not work well for one element array...")>>
    <<assert-false(cond="lower_bound(arr2, 8) == 2",code=12,message="`lower_bound` does not work well for one element array...")>>
    ! dup_arr
    dup_arr = [1, 1, 2, 3, 3, 3, 3, 5, 5, 5]
    <<assert-false(cond="lower_bound(dup_arr, 0) == 1",code=13,message="`lower_bound` does not work well for the array that has same values...")>>
    <<assert-false(cond="lower_bound(dup_arr, 2) == 3",code=13,message="`lower_bound` does not work well for the array that has same values...")>>
    <<assert-false(cond="lower_bound(dup_arr, 3) == 4",code=13,message="`lower_bound` does not work well for the array that has same values...")>>
    <<assert-false(cond="lower_bound(dup_arr, 5) == 8",code=13,message="`lower_bound` does not work well for the array that has same values...")>>
    <<assert-false(cond="lower_bound(dup_arr, 7) == 11",code=13,message="`lower_bound` does not work well for the array that has same values...")>>
    ! allsame_arr
    allsame_arr = [(1, i = 1, n)]
    <<assert-false(cond="lower_bound(allsame_arr, 0) == 1",code=13,message="`lower_bound` does not work well for the array that has all same values...")>>
    <<assert-false(cond="lower_bound(allsame_arr, 1) == 1",code=13,message="`lower_bound` does not work well for the array that has all same values...")>>
    <<assert-false(cond="lower_bound(allsame_arr, 2) == 11",code=13,message="`lower_bound` does not work well for the array that has all same values...")>>
...
#+RESULTS[8bdad10991244b1976c0a57ff71ca710c49b40fc]: test-vector
...
#+begin_src fortran :exports code :tangle "test/test_unwrapped_vector.F90" :noweb no-export
  program test_unwrapped_vector
    ...
    store:do i = 1, n
       call v%push_back(i)
       <<assert-false(cond="v%arr_(i) == i", code=10, message="Stored value in `v%arr_(i)` is illegal in loop.")>>
    end do store
    ...
    v2%arr_(:) = 1
    do i = 1, 5
       <<assert-false(cond="v2%arr_(i) == 1", code=20, message="Initialization by size of `v2` is illegal.")>>
    end do
    v2 = unwrapped_vector_int32([(i, i = 1,5)])
    do i = 1, 5
       <<assert-false(cond="v2%arr_(i) == i", code=21, message="Initialization by array of `v2` is illegal.")>>
    end do
    v2 = unwrapped_vector_int32(size = 5, val = 2)
    do i = 1, 5
       <<assert-false(cond="v2%arr_(i) == 2", code=22, message="Initialization by init_val of `v2` is illegal.")>>
    end do

    call v2%resize(0)
    do i = 1, 5
       call v2%push_back(i)
       <<assert-false(cond="v2%back() == i", code=23, message="Resize or back for `v2` is illegal.")>>
    end do
...
#+RESULTS[1a980011ba9f528ddfd7da7d9e1eb46a59c550e3]: test-unwrapped_vector
