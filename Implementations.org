...
**** The whole module of hash table
...
#+end_src
**** The type declaration of hash table
...
#+begin_src fortran :exports code
  subroutine insert_${binary_tree}(tree, val)
    ...
    if (.not. associated(tree%root_)) then
       tree%root_ => ${binary_tree_node}(val)
       tree%num_elems_ = 1
       return
    end if
    child => tree%root_
    do
       if (val == child%elem_) then
          ...
          if (.not. associated(child%left_)) then
             child%left_ => ${binary_tree_node}(val)
          ...
          if (.not. associated(child%right_)) then
             child%right_ => ${binary_tree_node}(val)
             tree%num_elems_ = tree%num_elems_ + 1
             return
          end if
          child => child%right_
...
#+RESULTS[77a553cd19dd723f34997f7ad7f3dd3d2efae278]: test-binary-tree
: |           10 |            1           1
: |            1          10 |            2           2
: |            1          10          17 |            3           2
