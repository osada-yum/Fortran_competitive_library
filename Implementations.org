...
#+begin_src fortran :exports code
  ...
  type :: ${pq}
     private
     type(unwrapped_vector_${suffix}) :: vec_
   contains
  ...
  subroutine push_${pq}(this, val)
    ...
    upheap:do ! reconstruct the heap by ${op}.
       if (i == 1) then ! top of the heap
          ...
          exit
       else if (tmp ${op} this%vec_%arr_(i/2)) then ! move the element up in the heap
...
#+begin_src fortran :exports code
  ...
  ${type} function pop_${pq}(this) result(res)
    ...
    downheap:do ! reconstruct the heap by ${op}.
       prev = next
       if (2*prev > n) exit
       if (this%vec_%arr_(2*prev)      ${op} this%vec_%arr_(next)) &
            next = 2*prev
       if (2*prev+1 <= n) then
          if (this%vec_%arr_(2*prev+1) ${op} this%vec_%arr_(next)) &
               next = 2*prev+1
       end if
...
#+RESULTS[d89dc881fe94f846f0270014c30b5b6888baf761]: test-priority_queue
