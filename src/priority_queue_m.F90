module priority_queue_m
  ...
  type :: priority_queue_min_int32
     private
     type(unwrapped_vector_int32) :: vec_
   contains
  ...
  type :: priority_queue_min_int64
     private
     type(unwrapped_vector_int64) :: vec_
   contains
  ...
  type :: priority_queue_min_real32
     private
     type(unwrapped_vector_real32) :: vec_
   contains
  ...
  type :: priority_queue_min_real64
     private
     type(unwrapped_vector_real64) :: vec_
   contains
  ...
  type :: priority_queue_min_character
     private
     type(unwrapped_vector_character) :: vec_
   contains
...
contains
  ...
  subroutine push_priority_queue_min_int32(this, val)
    ...
    integer(int32) :: i
    ! add  to heap.
    call this%vec_%push_back(val)
    i = this%vec_%size()
    tmp = val
    upheap:do ! reconstruct the heap by <.
       if (i == 1) then ! top of the heap
          this%vec_%arr_(1) = tmp
          exit
       else if (tmp < this%vec_%arr_(i/2)) then ! move the element up in the heap
          this%vec_%arr_(i) = this%vec_%arr_(i/2)
       else ! move the element up in the heap
          this%vec_%arr_(i) = tmp
          tmp = this%vec_%arr_(i/2)
       end if
       i = i / 2
    end do upheap
  ...
  integer(int32) function pop_priority_queue_min_int32(this) result(res)
    ...
    res = this%vec_%arr_(1)
    this%vec_%arr_(1) = this%vec_%pop_back()
    n = this%vec_%size()
    ...
    downheap:do ! reconstruct the heap by <.
       prev = next
       if (2*prev > n) exit
       if (this%vec_%arr_(2*prev)      < this%vec_%arr_(next)) &
            next = 2*prev
       if (2*prev+1 <= n) then
          if (this%vec_%arr_(2*prev+1) < this%vec_%arr_(next)) &
               next = 2*prev+1
       end if
       if (prev == next) exit ! arr(next) < arr(2*prev) .and. arr(next) < arr(2*prev+1)
       call swap(this%vec_%arr_(prev), this%vec_%arr_(next))
    end do downheap
  contains
    subroutine swap(x, y)
  ...
  subroutine dump_priority_queue_min_int32(this)
    class(priority_queue_min_int32), intent(in) :: this
    write(error_unit, '(*(g0, 1x))') this%vec_%arr_(1:this%size())
